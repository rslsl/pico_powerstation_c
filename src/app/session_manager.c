...
void session_manager_step(SessionManager *mgr,
    ...
    if (!mgr->prev_charge_active && snap->is_charging) {
        ...
        log_charge_start(logger, snap->soc, snap->voltage, snap->i_chg);
        ...
        log_charge_end(logger, snap->soc, session_wh);
    }
    ...
    if (!mgr->prev_discharge_active && discharge_active) {
        ...
        mgr->discharge_start_soc = snap->soc / 100.0f;
        ...
        float dod_frac = mgr->discharge_start_soc - (snap->soc / 100.0f);
        ...
        if (discharge_dur_ms >= RUNTIME_DISCHARGE_MIN_MS) {
            printf("[CYCLE] end dur=%lus DoD=%.0f%%\n",
                   ...
                   dod_frac * 100.0f);
            bat_cycle_end(bat);
            log_discharge_end(logger, snap->soc, session_wh);
